...
// Interactive 3D tilt around arbitrary card content: an intro wiggle on load
// (CSS, works everywhere), then follows the cursor on desktop and the device
// gyroscope on a phone. iOS needs motion permission, requested on a tap.
const MAX = 12;
const clamp = (v: number) => Math.max(-MAX, Math.min(MAX, v));

type DOE = typeof DeviceOrientationEvent & { requestPermission?: () => Promise<"granted" | "denied"> };

export function TiltWrap({ children, radius = 18 }: { children: ReactNode; radius?: number }) {
  ...
  return (
    <div ref={ref} onMouseMove={onMove} onMouseLeave={reset} onClick={enableGyro} style={{ perspective: 900 }}>
      <div
